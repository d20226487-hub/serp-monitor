...
melbet	kz	34	20,000	0.45`,
  ...
  positions: {
    ...
    fromRuns: (runs: number, rows: number) =>
      `${rows} keyword/SERP row(s) from ${runs} run(s) in this range`,
    colKeyword: "Keyword",
    colVariant: "SERP",
    colChecked: "Checked",
    footnote: "Each row shows its most recent run inside the range. A dash means the domain was not in the positions that run captured — which is not the same as not ranking at all.",
...
melbet	kz	34	20 000	0,45`,
  ...
  positions: {
    ...
    fromRuns: (runs: number, rows: number) =>
      `строк «запрос/выдача»: ${rows}, прогонов за период: ${runs}`,
    colKeyword: "Запрос",
    colVariant: "Выдача",
    colChecked: "Проверено",
    footnote: "В каждой строке — последний прогон внутри периода. Прочерк означает, что домена не было среди снятых позиций этого прогона, а это не то же самое, что «не ранжируется вовсе».",
