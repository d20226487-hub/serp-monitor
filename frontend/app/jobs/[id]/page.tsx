...
export default function JobPage() {
  ...
  if (!job) return <div className="text-sm text-slate-600 dark:text-slate-400">{t.common.loading}</div>;
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button className="text-sm px-3 py-1 border rounded dark:border-slate-700"
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={() => setEdit(true)} className="px-3 py-1.5 rounded-md border dark:border-slate-700 text-sm">{t.common.edit}</button>
      </div>
      ...
      <div className="space-y-2">
        {runs.length === 0 && <div className="text-sm text-slate-600 dark:text-slate-400">{t.jobs.noRuns}</div>}
        {runs.map(r => (
          <div key={r.id} id={`run-${r.id}`}
               className="border rounded-md px-4 py-3 flex flex-wrap items-center gap-3 dark:border-slate-700">
            <StatusBadge status={r.status} />
            <Link href={`/runs/${r.id}`} className="font-medium hover:underline">{t.jobs.runEntry.runLabel(r.id)}</Link>
            <div className="text-xs text-slate-600 dark:text-slate-400 flex-1 min-w-0 truncate">
              ...
              {r.cost != null && (
                <span title={r.cost_source === "actual" ? t.cost.actualHint : t.cost.estimateHint}>
                  {" · "}{formatUsd(r.cost)}
                  {r.cost_source === "estimate" && <span className="text-slate-500 dark:text-slate-400">*</span>}
                </span>
              )}
            </div>
            <Link href={`/runs/${r.id}`} className="text-sm px-2 py-1 rounded border dark:border-slate-700">{t.common.open}</Link>
          </div>
        ))}
      </div>
    </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="border rounded-md px-3 py-2 dark:border-slate-700">
...
function ScheduleField({ job, info }: { job: Job; info: ScheduleInfo | null }) {
  ...
  return (
    <div className="border rounded-md px-3 py-2 dark:border-slate-700">
      <div className="text-xs text-slate-600 dark:text-slate-400">{t.jobs.fields.schedule}</div>
      ...
      <div className="text-xs mt-0.5">
        {enabled
          ...
          : <span className="text-slate-600 dark:text-slate-400">{t.jobs.schedule.disabled}</span>}
